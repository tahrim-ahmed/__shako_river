...
//	$slab_id, $slab_gift, $r_total_next, $slab_id_next, $slab_gift_next,
    ...
    if(isset($_POST['search'])){
        $start=$_POST['start'];
        $end=$_POST['end'];
        $query = "SELECT retailer.*, call_info.* FROM retailer INNER JOIN call_info ON retailer.r_code = call_info.retailer_code WHERE (call_info.date BETWEEN '" . $start . "' AND '".$end."') AND (call_info.submitted_by = '$user')";
        $result = mysqli_query($conn, $query);
    }
    else{
        $query = "SELECT retailer.*, call_info.* FROM retailer INNER JOIN call_info ON retailer.r_code = call_info.retailer_code WHERE call_info.submitted_by = '$user'";
        $result = mysqli_query($conn, $query);
    }
    ?>
