...
if(isset($_POST['search']))
{
	...
	$sql10 = "SELECT SUM(i_amount) as own FROM invoice WHERE d_check='yes' and r_code='$r_code'";
	$sql11 = "SELECT SUM(i_amount) as other FROM invoice WHERE d_check='no' and r_code='$r_code'";
	$sql12 = "SELECT count(id) as count FROM invoice WHERE r_code='$r_code'";
	...
	$sql20 = "SELECT * FROM slab WHERE '$r_own' BETWEEN a_start and a_end";
	...
	if (mysqli_num_rows($slab)>0){
		while($row = mysqli_fetch_assoc($slab)) {
			...
			$a_slab= $row['a_slab'];
			$n_slab= $row['n_slab'];
			$n_a_slab= $row['n_a_slab'];
			$gift= $row['gift'];
			$a_gift= $row['a_gift'];
		}
	}

	$sql21 = "SELECT * FROM slab WHERE c_slab='$n_slab'";
	...
	if (mysqli_num_rows($slab_next)>0){
		while($row = mysqli_fetch_assoc($slab_next)) {
			...
			$slab_n_gift= $row['gift'];
			$r_a_start=$row['a_start']; 
			if ($n_slab=="Special") {
			$r_n_own="If Region Champion";
			}
			else{
			$r_n_own = $row['a_start']-$r_own;
		}
		}
	}
	
	$sql22 = "SELECT * FROM slab WHERE a_slab='$n_a_slab'";

 	$a_slab_next=mysqli_query($conn, $sql22);

	if (mysqli_num_rows($a_slab_next)>0){
		while($row = mysqli_fetch_assoc($a_slab_next)) {

			$a_slab_n_gift= $row['a_gift'];
			$r_a_n_start=$row['a_start'];
			if($n_a_slab=="A0"){
			$r_a_n_own="Not Applicable";
			}
			else{
			$r_a_n_own = $row['a_start']-$r_own;
		}
		}
	}
...
while($row = mysqli_fetch_assoc($result)) {
	...
	if(isset($_POST["submit"])){
		...
		//$error = $_POST['error'];
$sql3="SELECT r_code,date,memo_no,i_amount FROM invoice WHERE r_code='$r_code'AND date='$date'AND memo_no='$memo_no' AND i_amount='$i_amount'";


$sql2= "INSERT INTO invoice (r_code, d_check, date, memo_no, i_amount, p_amount,Remarks, entry, user, status, error)
...
<body>
    ...
    <li class="breadcrumb-item" aria-current="page" style="color:DodgerBlue;">Jan-Jun 2020</li>
  </ol>
...
if(isset($_POST['search']))
{
	$retailer_code=$_POST['typeahead'];
		...
		$sql4 = "SELECT * FROM `invoice` WHERE `r_code`='$r_code'";
...
if ($row['status']=="Verified" && $_SESSION['type'] == "user") {
			...
			echo "<td>
				<a href='add_data2.php?eid={$row['id']}'><i class='fas fa-edit' style='font-size:20px;color:blue;'></i></a> &nbsp
				<a href='add_data2.php?did={$row['id']}'><i class='fas fa-trash-alt' style='font-size:20px;color:red;'></i></a> &nbsp&nbsp
				<a href='add_data2.php?vid={$row['id']}'><i class='fas fa-check-square' style='font-size:21px;color:green;'></i></a>
			</td>";
}
...
if (isset($_GET['did'])) {
	$del=$_GET['did'];
	$sql3= "DELETE FROM invoice WHERE id='$del'";
	mysqli_query($conn, $sql3);
	echo "	<script type='text/javascript'>
			alert('Invoice $del; Delete Successful');
			window.location = 'add_data2.php';
		</script>";
}
...
if (isset($_GET['eid'])) {
	...
	if(isset($_POST["update"])){
		...
		$sql5="UPDATE invoice SET d_check='{$e_d_check}', date ='{$e_date}', memo_no='{$e_memo_no}', i_amount= '{$e_i_amount}', p_amount= '{$e_p_amount}', remarks= '{$e_remarks}', entry= '{$e_entry}',error= '{$e_error}', user= '{$e_name}', status= '{$e_status}'WHERE id='{$e_id}'";
		$update=mysqli_query($conn,$sql5);
			echo "<script type='text/javascript'>
			alert('Invoice:$edit;Update Successful.');
			window.location = 'add_data2.php';
		</script>";
...
$v_status = "Verified";
$sql6 ="UPDATE invoice SET status='{$v_status}', user='{$v_name}' WHERE id='{$verify}'";
$result6=mysqli_query($conn,$sql6);
	echo "<script type='text/javascript'>
	alert('Invoice:$verify; Verified');
	window.location = 'add_data2.php';
	</script>";
