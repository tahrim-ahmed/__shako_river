...
//	$slab_id, $slab_gift, $r_total_next, $slab_id_next, $slab_gift_next,
    ...
    if(isset($_POST['search']))
    {
        ...
        $sql10 = "SELECT SUM(i_amount) as own FROM invoice3 WHERE d_check='yes' and r_code='$r_code'";
        $sql11 = "SELECT SUM(i_amount) as other FROM invoice3 WHERE d_check='no' and r_code='$r_code'";
        $sql12 = "SELECT count(id) as count FROM invoice3 WHERE r_code='$r_code'";
...
//Slab Calculation
    ...
    if(isset($_POST["submit"])){
        ...
        //$error = $_POST['error'];
        $sql3="SELECT r_code,date,memo_no,i_amount FROM invoice3 WHERE r_code='$r_code'AND date='$date'AND memo_no='$memo_no' AND i_amount='$i_amount'";


        $sql2= "INSERT INTO invoice3 (r_code, d_check, date, memo_no, i_amount, p_amount,Remarks, entry, user, status, error)
VALUES ('$r_code', '$d_check','$date','$memo_no', '$i_amount', '$p_amount', '$remarks', '$entry','$name','$status','$error' )";
    ...
    if (isset($_POST["missing"])) {
        ...
        $find = "SELECT r_cell FROM retailer WHERE r_code = '$r_code'";
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page" style="color:DodgerBlue;">Jan-Apr 2021</li>
        </ol>
    </nav>
        ...
        <div class="table-responsive-sm">
                        ...
                        <div class="card">
                            ...
                            <fieldset>
                                ...
                                <form action="" method="post">
                                    ...
                                    <tr>
                                        <td colspan="3" style="text-align: center;">
                                            <input class="btn btn-success btn-sm btn-block" type="submit" name="missing" value="Missing Number">
                                        </td>
                                    </tr>
                                </form>
                            </fieldset>
                            ...
                            <fieldset>
                                ...
                                <table class="table table-sm table-hover table-bordered">
                                    ...
                                    if(isset($_POST['search']))
                                    {
                                        ...
                                        $sql4 = "SELECT * FROM `invoice3` WHERE `r_code`='$r_code'";
			...
			";
				...
				<a href='add_data3.php?vid={$row['id']}'><i class='fas fa-check-square' style='font-size:21px;color:green;'></i></a>
                                    ...
                                    if (isset($_GET['did'])) {
                                        $del=$_GET['did'];
                                        $sql3= "DELETE FROM invoice3 WHERE id='$del'";
                                        mysqli_query($conn, $sql3);
                                        echo "	<script type='text/javascript'>
			alert('invoice3 $del; Delete Successful');
			window.location = 'add_data3.php';
		</script>";
                                    }
	...
	<tr>
		...
		<input type='submit' name='update' class='btn btn-success' value='Update'>
                                        ...
                                        if(isset($_POST["update"])){
                                            ...
                                            $sql5="UPDATE invoice3 SET d_check='{$e_d_check}', date ='{$e_date}', memo_no='{$e_memo_no}', i_amount= '{$e_i_amount}', p_amount= '{$e_p_amount}', remarks= '{$e_remarks}', entry= '{$e_entry}',error= '{$e_error}', user= '{$e_name}', status= '{$e_status}'WHERE id='{$e_id}'";
                                            $update=mysqli_query($conn,$sql5);
                                            echo "<script type='text/javascript'>
			alert('invoice3:$edit;Update Successful.');
			window.location = 'add_data3.php';
		</script>";
                                    ...
                                    if (isset($_GET['vid'])) {
                                        ...
                                        $v_status = "Verified";
                                        $sql6 ="UPDATE invoice3 SET status='{$v_status}', user='{$v_name}' WHERE id='{$verify}'";
                                        $result6=mysqli_query($conn,$sql6);
                                        echo "<script type='text/javascript'>
	alert('invoice3:$verify; Verified');
	window.location = 'add_data3.php';
	</script>";
